...
import { Seo, Icon, Spinner } from '../components';
...
const StatsSkeleton = () => (
  <div className="incident-stats page-shell">
    <header className="page-header">
      <div>
        <span className="dashboard__skeleton-block dashboard__skeleton-block--sm" style={{ marginBottom: '8px' }} />
        <span className="dashboard__skeleton-block dashboard__skeleton-block--lg" style={{ height: '32px' }} />
      </div>
    </header>
    ...
    <div className="stats-grid">
      {[1, 2, 3, 4].map(i => (
        <div key={i} className="stat-card stat-card--skeleton">
          <div className="stat-card__icon skeleton-block" />
          <div className="stat-card__data">
            <span className="dashboard__skeleton-block dashboard__skeleton-block--sm" style={{ height: '24px', marginBottom: '4px' }} />
    ...
    <div className="stats-charts">
      {[1, 2].map(i => (
        <div key={i} className="chart-container card">
          <span className="dashboard__skeleton-block dashboard__skeleton-block--md" style={{ marginBottom: '20px' }} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {[1, 2, 3, 4].map(j => (
              <div key={j} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span className="dashboard__skeleton-block" style={{ width: '80px' }} />
                <span className="dashboard__skeleton-block" style={{ flex: 1 }} />
                <span className="dashboard__skeleton-block" style={{ width: '20px' }} />
              </div>
            ))}
          </div>
        </div>
